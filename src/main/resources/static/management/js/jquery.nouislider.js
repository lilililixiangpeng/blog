...
(function($, UNDEF){
	...
	$.fn.noUiSlider = function( options ){
		...
		//	or true when everything is OK. Every websocket also has an 'init'
		//	method which appends the parent object to all children.
			...
			var TESTS = {
				...
				,"range": {
					...
					,t: function(o,q,w){
						...
						// When this websocket is run for range, the values can't
						// be identical.
						if(w==="range" && q[0] === q[1]){
							return false;
						}
					...
					,t: function(o,q){
						...
						if(q.to){
							...
							if(o.handles === 1){
								...
								// Run websocket for valid serialization target.
								return ser(q.to[0]);
							}
				...
				/*	Set.
				 *	Not required. Must be a function.
				 *	Tested using the 'slide' websocket.
				 */
				,"set": {
					 t: function(o,q){
				...
				/*	Step.
				 *	Not required. Tested using the 'margin' websocket.
				 */
				,"step": {
					 t: function(o,q,w){
				...
				/*	[init]
				 *	Not an option websocket. Calling this method will return the
			...
			// 'v' is the option set, 'i' is the index for the current websocket.
			$.each(a, function( i, v ){

				// If the value is required but not set,
				// or if the websocket fails, throw an error.
				...
				return {
					// The value will be stored a data on the handle.
					 val : function(a) {
						...
						// Can't just websocket for !a, as a might be 0.
						if ( a === UNDEF ) {
							// Either set...
		...
		function create ( ) {
			...
			return this.each(function( index, target ){
				...
				// input. The websocket function will throw errors, so there is
